...
class PollOfficer extends Component {
	constructor(props) {
		super(props);
		this.state = {
			elections: []
	...
	render() {
		if (this.props.auth.user.role !== 'poll-officer') {
			return <Redirect to="/" />;
		}
		return (
			<Grid>
					...
					<ListGroup>
						{this.state.elections.map((election) => (
							<ListGroupItem key={election.key}>
								<Link to={`/poll-officer/${election.key}`}>{election.name}</Link>
							</ListGroupItem>
						))}
					</ListGroup>
				</Row>
...
export default connect(mapStateToProps)(PollOfficer);
